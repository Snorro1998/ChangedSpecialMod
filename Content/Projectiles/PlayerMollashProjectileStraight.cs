...
{
    public class PlayerMollashProjectileStraight : ModProjectile
    {
        ...
        public override void AI()
        {
            ...
            for (int i = 0; i < segmentPositions.Count; i++)
            {
                ...
                // Cut tiles (grass, vines, pots, etc.)
                Vector2 start = pos - new Vector2(6, 6);
                Vector2 end = pos + new Vector2(6, 6);

                DelegateMethods.tilecut_0 = TileCuttingContext.AttackProjectile;
                Utils.PlotTileLine(start, end, 12, DelegateMethods.CutTiles);
            }
        }
        ...
        private void GetPositionAndRotationLists(Player owner, float currentRotation, float rotationPerSegment, float baseRotation, float length, out List<Vector2> segmentPositions, out List<float> segmentRotations)
        {
