...
{
    public class NPCMollashProjectile : ModProjectile
    {
        private const int SegmentCount = 10;
        private const float MaxLength = 150;
        private const int AnimationTime = 30;
        private const float SegmentDist = 18;
        private const float RotPerSegment = 0.3f;
        ...
        public override void AI()
        {
            ...
            Vector2 baseDir = baseRotation.ToRotationVector2();
            float arcDirection = baseDir.X >= 0f ? 1f : -1f;

            float currentRotation = baseRotation +
                MathHelper.Lerp(-arc / 2f, arc / 2f, progress) * arcDirection;

            float rotationPerSegment =
                MathHelper.Lerp(-RotPerSegment, RotPerSegment, progress) * arcDirection;
            ...
            List<int> struckNPCs = new List<int>();
        ...
        private void GetPositionAndRotationLists(NPC owner, float currentRotation, float rotationPerSegment, out List<Vector2> segmentPositions, out List<float> segmentRotations)
        {
            ...
            float incrementBase = 1f;
            float incrementPerSegment = 0.13f;
            float accumulatedRotation = currentRotation;

            for (int i = 0; i < SegmentCount; i++)
            {
                float increment = incrementBase + incrementPerSegment * i;
                accumulatedRotation += rotationPerSegment * increment;

                Vector2 direction = accumulatedRotation.ToRotationVector2();
                position += direction * SegmentDist;

                segmentPositions.Add(position);
                segmentRotations.Add(accumulatedRotation);
            }
        }
        ...
        public override bool PreDraw(ref Color lightColor)
        {
            ...
            // Flip arc if facing left
            float arcDirection = baseDir.X >= 0f ? 1f : -1f;

            float currentRotation = baseRotation + MathHelper.Lerp(-arc / 2f, arc / 2f, progress) * arcDirection;
            ...
            var rotationPerSegment = MathHelper.Lerp(-RotPerSegment, RotPerSegment, progress) * arcDirection;
            var lastPosition = owner.Center;
            var lastRotation = currentRotation;

            GetPositionAndRotationLists(owner, currentRotation, rotationPerSegment, out segmentPositions, out segmentRotations);
            DrawLine(segmentPositions);

            for (int i = 0; i < segmentPositions.Count; i++)
            {
                var color = Lighting.GetColor(segmentPositions[i].ToTileCoordinates()/*, Color.White*/);
                var pos = segmentPositions[i];
                var rotation = segmentRotations[i] - MathHelper.PiOver2;

                Rectangle frame = new Rectangle(0, 18, 18, 18);
                Vector2 origin = new Vector2(5, 8);

                // Tip of the tentacle has a different sprite
                if (i == SegmentCount - 1)
                {
                    frame.Y = 68;
                    frame.Height = 18;
