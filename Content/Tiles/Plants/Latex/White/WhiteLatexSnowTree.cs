...
{
    public class WhiteLatexSnowTree : ModTree
    {
        ...
        {
            GrowsOnTileId = [ModContent.TileType<WhiteLatexSnowTile>()];
        ...
        {
            style = 0;
            return ModContent.TileType<WhiteLatexSnowTreeSapling>();
        }
        ...
        private void SpawnLatex(IEntitySource source, Player player, int x, int y, bool isDrunk)
        {
            var xPos = x * 16;
            var yPos = y * 16;
            var npcType = !isDrunk ? ChangedUtils.Choose(ModContent.NPCType<WhiteGoop>()) : ChangedUtils.Choose(ModContent.NPCType<QuackLatex>());
