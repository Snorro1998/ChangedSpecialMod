...
namespace ChangedSpecialMod.Content.Tiles.Plants
{
    public class WhiteLatexPalmTree : ModPalmTree
    {
        ...
        public override void SetStaticDefaults()
        {
            ...
            texture = ModContent.Request<Texture2D>("ChangedSpecialMod/Content/Tiles/Plants/WhiteLatexPalmTree");
            branchesTexture = ModContent.Request<Texture2D>("ChangedSpecialMod/Content/Tiles/Plants/WhiteLatexTree_Branches");
            topsTexture = ModContent.Request<Texture2D>("ChangedSpecialMod/Content/Tiles/Plants/WhiteLatexTree_Tops");
            oasisTopsTexture = ModContent.Request<Texture2D>("ChangedSpecialMod/Content/Tiles/Plants/WhiteLatexPalmTree_Tops");
        }
        ...
        private void SpawnLatex(IEntitySource source, Player player, int x, int y, bool isDrunk)
        {
            var xPos = x * 16;
            var yPos = y * 16;
            var npcType = !isDrunk ? ChangedUtils.Choose(ModContent.NPCType<WhiteGoop>()) : ChangedUtils.Choose(ModContent.NPCType<FlightLatex>());
        ...
        {
            return ModContent.GoreType<WhiteLatexTreeLeaf>();
        }
    }
}
