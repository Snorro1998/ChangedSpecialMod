...
{
	public class PackingBox : ModTile
	{
		public override void SetStaticDefaults() {
			...
			RegisterItemDrop(ModContent.ItemType<Items.Placeable.Furniture.PackingBox>(), 1);
		...
		public override bool RightClick(int i, int j) {
			...
			if (Main.netMode == NetmodeID.MultiplayerClient && !isLocked) {
				...
				if (isLocked) {
					...
					int key = ModContent.ItemType<PackingBoxKey>();
		...
		public override void MouseOver(int i, int j) {
			...
			if (chest < 0) {
				...
				if (player.cursorItemIconText == defaultName) {
					player.cursorItemIconID = ModContent.ItemType<Content.Items.Placeable.Furniture.PackingBox>();
					if (Main.tile[left, top].TileFrameX / 36 == 1) {
						player.cursorItemIconID = ModContent.ItemType<PackingBoxKey>();
					}
