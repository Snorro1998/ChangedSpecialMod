...
namespace ChangedSpecialMod.Content.NPCs
{
    ...
    public abstract class Worm2 : ModNPC
    {
        ...
        /*
        // Destroyer drawing code, taken from DrawNPCDirect_Inner
        public override bool PreDraw(SpriteBatch spriteBatch, Vector2 screenPos, Color drawColor)
        {
            ...
            var textureAsset = TextureAssets.Npc[NPC.type];
            ...
            Vector2 halfSize = new Vector2(textureAsset.Width() / 2, textureAsset.Height() / Main.npcFrameCount[NPC.type] / 2);
            spriteBatch.Draw(texture, 
                new Vector2(NPC.position.X - screenPos.X + (float)(NPC.width / 2) - (float)textureAsset.Width() * NPC.scale / 2f + halfSize.X * NPC.scale, 
                NPC.position.Y - screenPos.Y + (float)NPC.height - (float)textureAsset.Height() * NPC.scale / (float)Main.npcFrameCount[NPC.type] + 4f + halfSize.Y * NPC.scale + num36 + num35),
                NPC.frame, new Color(255, 255, 255, 0) * (1f - (float)NPC.alpha / 255f), 
                NPC.rotation, halfSize, NPC.scale, spriteEffects, 0f);
            return false;
        }
        */
        ...
        public sealed override bool PreAI()
        {
            ...
            var reverseMovement = NPC.ai[2] > 300;
            ...
            if (NPC.ai[2] == 300)
            {
                ...
                ForcedTargetPosition = new Vector2(NPC.position.X, Main.maxTilesY * 16);
            }
        ...
        protected int SpawnSegment(IEntitySource source, int type, int latestNPC)
        {
            //(int)NPC.Center.Y
            // Decoil the worm
            var yPos = (int)(Main.npc[latestNPC].Center.Y + 16 * 10);
            ...
            latestNPC = NPC.NewNPC(source, (int)NPC.Center.X, yPos, type, NPC.whoAmI, 0, latestNPC);
    ...
    /// </summary>
    public abstract class WormHead2 : Worm2
    {
        ...
        {
            WormBody2.CommonAI_BodyTail(this);
        }
    }

    public abstract class WormBody2 : Worm2
    {
        ...
        internal static void CommonAI_BodyTail(Worm2 worm)
        {
            ...
            var reverseMovement = worm.NPC.ai[2] > 300;

            NPC following = worm.NPC.ai[1] >= Main.maxNPCs ? null : worm.FollowingNPC;
            if (reverseMovement)
                following = worm.NPC.ai[0] >= Main.maxNPCs ? null : worm.FollowerNPC;
            ...
            if (following is not null)
            {
                ...
                float dist = (length - worm.NPC.height) / length;//width
    ...
    public abstract class WormTail2 : Worm2
    {
