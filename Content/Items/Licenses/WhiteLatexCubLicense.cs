...
{
    public class WhiteLatexCubLicense : ModItem
    {
        ...
        {
            int npcType = ModContent.NPCType<WhiteLatexCubTownPet>();
            if (player.ItemAnimationJustStarted && (!TownPetSystem.boughtWhiteLatexCubPet || NPC.AnyNPCs(npcType)))
            {
                if (player.whoAmI == Main.myPlayer)
                {
                    UnlockPet(ref TownPetSystem.boughtWhiteLatexCubPet, npcType, this.GetLocalizationKey("UseWhiteLatexCubLicense")); // Modified NPC.UnlockOrExchangePet method.
                }
                return true;
            }
            return false;
        ...
        public static void UnlockPet(ref bool petBoughtFlag, int npcType, string textKeyForLicense)
        {
            ...
            if (Main.netMode == NetmodeID.MultiplayerClient)
            {
                if (!petBoughtFlag || NPC.AnyNPCs(npcType))
                {
                    ...
                    packet.Write((byte)ChangedSpecialMod.MessageType.WhiteLatexCubTownPetUnlock);
                    packet.Send();
                }
            }
