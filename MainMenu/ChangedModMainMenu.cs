...
namespace ChangedSpecialMod.MainMenu
{
    public class ChangedSpecialModMainMenu : ModMenu
    {
        ...
        public override string DisplayName => "Changed Style";

        public override Asset<Texture2D> Logo => ModContent.Request<Texture2D>("ChangedSpecialMod/MainMenu/Logo");
        public override Asset<Texture2D> SunTexture => ModContent.Request<Texture2D>("ChangedSpecialMod/Patches/BlankPixel");
        public override Asset<Texture2D> MoonTexture => ModContent.Request<Texture2D>("ChangedSpecialMod/Patches/BlankPixel");

        public override ModSurfaceBackgroundStyle MenuBackgroundStyle => ModContent.GetInstance<NullSurfaceBackground>();

        // Before drawing the logo, draw the entire background. This way, the typical parallax background is skipped entirely.
        public override bool PreDrawLogo(SpriteBatch spriteBatch, ref Vector2 logoDrawCenter, ref float logoRotation, ref float logoScale, ref Color drawColor)
        {
            Texture2D texture = ModContent.Request<Texture2D>("ChangedSpecialMod/MainMenu/MenuBackground").Value;
            ...
            static Color selectCinderColor()
            {
                ...
                return Color.Lerp(Color.Orange, Color.Yellow, Main.rand.NextFloat(0.9f));
            }
            ...
            // Draw cinders.
            Texture2D cinderTexture = ModContent.Request<Texture2D>("ChangedSpecialMod/Patches/CalamitasCinder").Value;
            for (int i = 0; i < Cinders.Count; i++)
            {
